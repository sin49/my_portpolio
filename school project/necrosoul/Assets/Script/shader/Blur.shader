    ...
    Properties
    {
		...
		half4 frag(v2f i) :SV_Target{
			...
			for (float x = -blur; x <= blur; x += 1) {
				...
				weight_total += weight;
				col += tex2Dproj(_GrabTexture, i.grabPos + float4(x * _GrabTexture_TexelSize.x, 0, 0, 0));
			}
			col /= weight_total;
			return col;
		}
		...
		half4 frag(v2f i) :SV_Target{
			...
			for (float y = -blur; y <= blur; y += 1) {
				...
				weight_total += weight;
				col += tex2Dproj(_GrabTexture, i.grabPos + float4(0,y * _GrabTexture_TexelSize.y, 0, 0));
			}
			col /= weight_total;
			return col;
		}
