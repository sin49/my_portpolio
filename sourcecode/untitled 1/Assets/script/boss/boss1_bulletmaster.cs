...
public class boss1_bulletmaster : MonoBehaviour//스테이지 1 보스 공격 클래스
{
    ...
    void Update()
    {
        if (!GetComponent<boss_basic>().die_check)
        {
            ...
            {
                //일저 간격으로 패턴 1과패턴2를 쓴다
                //체력이 절반이 되면 패턴 1과 패턴 2를 쓰는 간격이 빨라진다
                pattern1_time_check += Time.deltaTime;
                pattern2_time_check += Time.deltaTime;
                //pattern1_break를 통해 패턴 1을 쓴 후의 딜레이를 가진다

                //패턴 1은 체력이 절반이 되기전에는 3번 반복한 후 딜레이를 가진다
                if (!pattern1_break)
                {
                    if (pattern1_time_check >= 2)//2초 간격
                    {
                        enemy_time += Time.deltaTime;
                        if (pattern1_check <= 5)//pattern1을 pattern1_check만큼 실행
                        {
                            pattern1();
                    ...
                    if (pattern1_loop == 3)//pattern1_loop의 수만큼 패턴1( pattern1을 pattern1_check만큼 실행)을 반복한다
                    {
                        pattern1_loop = 0;
                        pattern1_break = true;
                    }
                ...
                {
                    if (GetComponent<boss_basic>().e_hp < GetComponent<boss_basic>().e_hp_max / 2)//피가 절반이라면 딜레이없이 패턴1을 실행
                    {
                        pattern1_break = false;
                    }
                    else
                    {
                        if (pattern1_time_check >= 2)//아니라면 2초 간격
                        {
                            pattern1_break = false;
                            pattern1_time_check = 0;
                        }
                ...
                if (GetComponent<boss_basic>().e_hp < GetComponent<boss_basic>().e_hp_max / 2)
                {
                    if (pattern2_time_check >= 0.25)//체력이 절반일때 0.25초 간격
                    {
                        pattern2();
                ...
                {
                    if (pattern2_time_check >= 0.5)//아닐시 0.5초 간격
                    {
                        pattern2();
                        pattern2_time_check = 0;
                    }
    ...
    public void pattern1()//두 위치에서 속도가 다른 조준탄이 각각 5개씩 나온다
    {
        //pattern1 마다 2개를 생성 pattern1_check만큼 반복하는 것으로 패턴1구현
        if (enemy_time >= 0.2)
        {
    ...
    public void pattern2() {//원형으로 퍼져나가는 탄막을 발사한다
